...
const OUTPUT_PATH = path.join(PROJECT_ROOT, "public", "dia-das-maes", "featured-products.json");
 ...
 * Preço, Pix, título e imagem descrevem o mesmo produto num mesmo instante: ou
 * vêm todos da leitura nova, ou vêm todos do snapshot.
 */
...
export const buildFeaturedProductsData = async ({
  ...
  for (const source of featuredSources) {
    ...
    try {
      ...
      if (fallbackProduct) {
        logger.warn(
          `[mothers-day] fallback used for ${url}: ${error instanceof Error ? error.message : String(error)}`,
        );
      ...
      logger.warn(
        `[mothers-day] product skipped for ${url}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
...
export const syncFeaturedProducts = async ({
  ...
  logger.info(`[mothers-day] wrote ${featuredProducts.length} featured products to ${outputPath}`);
  logger.info(`[mothers-day] wrote fallback snapshot to ${snapshotPath}`);
...
if (isDirectExecution) {
  syncFeaturedProducts().catch((error) => {
    console.error("[mothers-day] sync failed", error);
    process.exitCode = 1;
  });
}
