...
- Conversões: `trackStoreAction` chama `gtag("event", "conversion", { send_to, transaction_id: event_id })` usando `STORE_ADS_SEND_TO` em `src/lib/tracking.ts`. Com o valor vazio, nada é enviado. Preencher com o `send_to` do snippet de evento de cada ação criada no Google Ads (ligação e rota). Não colar o snippet de evento (`gtag_report_conversion`) na página: ele troca a navegação dos links.
